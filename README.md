...
> Full-text search service with multi-language support and search analytics
...
The Search Service is a FastAPI-based microservice that handles [describe main functionality]. It's part of the Mission Engadi platform, which aims to [mission statement].
...
```bash
git clone https://github.com/mission-engadi/search_service.git
cd search_service
```
...
# Database
DATABASE_URL="postgresql+asyncpg://postgres:postgres@localhost:5432/search_service_db"
...
- **OpenAPI Schema**: `http://localhost:8011/api/v1/openapi.json`
...
```bash
curl -H "Authorization: Bearer <token>" http://localhost:8011/api/v1/examples/
```
...
```bash
fly launch --name search_service
```
...
```bash
fly postgres create --name search_service-db
fly postgres attach search_service-db
```
...
- **Issues**: [GitHub Issues](https://github.com/mission-engadi/search_service/issues)
